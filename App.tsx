...
export default function App() {
  ...
  const [subModePrompt, setSubModePrompt] = useState('');
  ...
  const isCustomMode = selectedMode === 'Personalizado';
  const currentModeConfig = modes[selectedMode];
  const hasSubModes = !!currentModeConfig.subModes;
  const currentSubModes = hasSubModes ? Object.keys(currentModeConfig.subModes) : [];
  const isCustomSubMode = subMode === 'Personalizado';

  useEffect(() => {
    if (hasSubModes) {
      setSubMode(Object.keys(currentModeConfig.subModes)[0]);
    } else {
      setSubMode(null);
    }
    setSubModePrompt('');
  ...
  const onModeSelect = async (mode) => {
    ...
    if (isCustomMode) {
      promptInput = customPrompt;
       ...
       promptInput = isCustomSubMode ? subModePrompt : modes[mode].subModes[subMode];
    }

    if (mode === 'Gráfico') {
      setChartLabel(isCustomSubMode ? subModePrompt : subMode);
    }

    const prompt = typeof modes[mode].prompt === 'function'
        ? modes[mode].prompt(promptInput)
        : modes[mode].prompt;
        ...
        {vidUrl && !isLoadingVideo && (
          <>
            <div className={c('modeSelector', {hide: !showSidebar})}>
              {hasSubModes || isCustomMode ? (
                <>
                  <div>
                    {isCustomMode ? (
                      <>
                        <h2>Prompt personalizado:</h2>
                        <textarea
                          placeholder="Digite um prompt personalizado..."
                          value={customPrompt}
                      ...
                      <>
                        <h2>{selectedMode === 'Gráfico' ? 'Criar gráfico por:' : 'Tipo de tabela:'}</h2>
                        ...
                        <textarea
                          className={c({active: isCustomSubMode})}
                          placeholder="Ou digite um prompt personalizado..."
                          value={subModePrompt}
                          onChange={(e) => setSubModePrompt(e.target.value)}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                              onModeSelect(selectedMode);
                          ...
                          onFocus={() => setSubMode('Personalizado')}
                          rows="2"
                        />
                      </>
                    )}
